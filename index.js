...
app.post('/blog', isLoggedIn, upload.fields([{
    name: 'image',
    maxCount: 1
}, {
    name: 'thumbnail',
    ...
    try {
        ...
        const thumbnail = req.files['thumbnail'] ? req.files['thumbnail'][0].filename : null; // Retrieve the uploaded thumbnail filename or set it to null if no file is uploaded
        ...
        connection.query(checkUserQuery, [userId], (err, rows) => {
            ...
            const query = 'INSERT INTO blog_posts (title, description, image, thumbnail, user_id) VALUES (?, ?, ?, ?, ?)';
            connection.query(query, [title, description, image, thumbnail, userId], (err, result) => {
                if (err) {
...
app.put('/blog/:id', isLoggedIn, upload.single('image'), async (req, res) => {
    try {
        ...
        const query = 'UPDATE blog_posts SET title = ?, description = ?, image = ?, thumbnail = ? WHERE id = ? AND user_id = ?';
        const userId = req.session.user_id; // Retrieve the user ID from the session
        connection.query(query, [title, description, image, thumbnail, postId, userId], (err, result) => {
            if (err) {
...
app.get('/blog', isLoggedIn, async (req, res) => {
    try {
        ...
        const query = 'SELECT * FROM blog_posts WHERE user_id = ? AND isActive = true';
...
app.get('/blog/:id', isLoggedIn, async (req, res) => {
    try {
        ...
        const query = 'SELECT * FROM blog_posts WHERE id = ? AND user_id = ? AND isActive = true';
        connection.query(query, [postId, userId], (err, results) => {
            if (err) {
                console.error('Error retrieving blog post:', err);
                res.status(500).send('Internal server error');
...
app.put('/blog/:id/active', isLoggedIn, async (req, res) => {
    try {
        ...
        const checkOwnershipQuery = 'SELECT * FROM blog_posts WHERE id = ? AND user_id = ? ';
        connection.query(checkOwnershipQuery, [postId, userId], (err, results) => {
            ...
            connection.query(updateQuery, [isActive, postId], (updateErr, updateResult) => {
                if (updateErr) {
